...
namespace CppBuild.Graph
{
    ...
    public class LocalExecutor : TaskExecutor
    {
        ...
        /// </summary>
        public float ProcessorCountScale;

        public LocalExecutor(int threadCountMax, float processorCountScale)
        {
            ThreadCountMax = threadCountMax;
            ProcessorCountScale = processorCountScale;
        }
        ...
        public override int Execute(List<Task> tasks)
        {
            ...
            Log.Info(string.Format("Executing {0} {2} using {1} {3}", count, threadsCount,
                count == 1 ? "task" : "tasks", threadsCount == 1 ? "thread" : "threads"));
        ...
        private void ThreadMain(object obj)
        {
            ...
            while (true)
            {
                ...
                if (taskToRun != null)
                {
                    ...
                    if (result != 0)
                    {
                        Log.Error(string.Format("Task {0} {1} failed with exit code {2}", taskToRun.CommandPath,
                            taskToRun.CommandArguments, result));
                        Log.Error("");
                    }
