...
namespace CppBuild
{
    ...
    public static class Profiling
    {
        ...
        /// </summary>
        public static void GenerateTraceEventFile(string traceEventFilePath)
        {
            var path = Path.GetDirectoryName(traceEventFilePath);
            if (!string.IsNullOrEmpty(path) && !Directory.Exists(path))
                Directory.CreateDirectory(path);

            Log.Info("Saving trace events to " + traceEventFilePath);
            ...
            File.WriteAllText(traceEventFilePath, contents.ToString());
        }
    }
}